...
ds = xr.open_dataset(
    "../dados/netcdf/Umidade.Solo.amazonia.2019.2020.nc", decode_times=False
)
...
shape_bioma = ShapelyFeature(
    Reader(f"../dados/shapefile/bioma_amazonia/states_amazon_biome.shp").geometries(),
    crs.PlateCarree(),
    facecolor="none",
)
...
fig, ax = plot.subplots(
    axheight=5, nrows=nlinhas, ncols=ncolunas, tight=True, proj="pcarree"
)

# Formatação do mapa.
ax.format(
    coast=False, borders=False, grid=False, latlim=(latS, latN), lonlim=(lonW, lonE)
)
...
for i in range(1, total_paineis):
    ...
    # Plot da variável.
    map = ax[i - 1].pcolormesh(
        umidade_solo["lon"],
        umidade_solo["lat"],
        umidade_solo["sfsm"][i - 1, :, :],
        cmap=cores,
        levels=valores_de_umidade,
        extend="both",
    )
    ...
    ax[i - 1].add_feature(shape_bioma, linewidth=4.0, edgecolor="k")

#  Adiciona a barra de cores.
x = fig.colorbar(
    map, loc="b", width="4em", extendsize="12em", shrink=0.8, ticks=valores_de_umidade
)
...
fig.save("ex07.jpg", transparent=True, dpi=300, bbox_inches="tight", pad_inches=0.1)
