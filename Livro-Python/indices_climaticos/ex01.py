...
# A Função retorna o dado para o formato espacial (time, lat, lon).
...
# 1) Dado espacial:
# Inserindo atributos.
spi.spi.attrs["long_name"] = f"SPI - {scale} meses;"
...
encoding1 = {
    "spi": {"_FillValue": -999, "complevel": 9, "zlib": True},
...
spi.to_netcdf(path="spi_espacial.nc", unlimited_dims={"time": True}, encoding=encoding1)

# 2) Série temporal:
# Média de todas as latitudes e longitudes gerando uma série temporal.
x = spi.mean(dim=("lon", "lat"))  # time: 492

# Inserindo atributos.
x.spi.attrs["long_name"] = f"SPI - {scale} meses;"
...
# Salva o resultado no NetCDF.
x.to_netcdf(path="spi_serie.nc", unlimited_dims={"time": True}, encoding=encoding2)
